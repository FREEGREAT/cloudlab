...
resource "aws_api_gateway_model" "post_courses" {
  rest_api_id  = aws_api_gateway_rest_api.this.id
...
##############################################
#onecourse
resource "aws_api_gateway_method" "get_one_course" {
  authorization = "NONE"
  http_method   = "GET"
...
resource "aws_api_gateway_integration" "get_one_course" {
  rest_api_id             = aws_api_gateway_rest_api.this.id
  resource_id             = aws_api_gateway_resource.course.id
  http_method             = aws_api_gateway_method.get_one_course.http_method
  integration_http_method = "POST"
  type                    = "AWS"
  uri                     = module.lambdas.lambda_get_one_course_invoke_arn
  request_parameters      = { "integration.request.header.X-Authorization" = "'static'" }
  request_templates = {
...
resource "aws_api_gateway_method_response" "get_one_course" {
  rest_api_id     = aws_api_gateway_rest_api.this.id
  resource_id     = aws_api_gateway_resource.course.id
  http_method     = aws_api_gateway_method.get_one_course.http_method
  status_code     = "200"
...
resource "aws_api_gateway_integration_response" "get_one_course" {
  rest_api_id = aws_api_gateway_rest_api.this.id
  resource_id = aws_api_gateway_resource.course.id
  http_method = aws_api_gateway_method.get_one_course.http_method
  status_code = aws_api_gateway_method_response.get_one_course.status_code

  depends_on = [
    aws_api_gateway_method.get_one_course,
    aws_api_gateway_integration.get_one_course
  ]
}
